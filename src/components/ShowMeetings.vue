                    ...
                    <table>
                        ...
                        <tbody>
                            <tr class="meeting-data" v-for="(meeting, index) in meetings" :key="index">
                                ...
                                <td>{{ meeting.startTime.split(' ')[1] }}</td>
                                <td>{{ meeting.endTime.split(' ')[1] }}</td>
                                <td><a href="" class="delete" @click="handleDelete(meeting.id)">Delete</a></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
<script>
import MeetingService from '@/apis/MeetingService'
                ...
                MeetingService.getJoinedMeetings(localStorage.getItem('uid'))
                    .then((res) => {
                        ...
                        if (!this.meetings.length) {
                            this.message = "You are not in any meetings"
                        }
                    })
                    .catch((err) => {
                        console.log(err)
